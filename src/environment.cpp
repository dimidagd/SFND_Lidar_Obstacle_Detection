...
//void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud = pointProcessorI->SegmentPlaneOwn(filteredCloud, 200, 0.2);
...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr latest_scan = lidar->scan();
    //renderRays(viewer,lidar->position ,latest_scan);
    //renderPointCloud(viewer, latest_scan, "latest_scan");
    // TODO:: Create point processor
    ProcessPointClouds<pcl::PointXYZ>processor ;
    std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr> segmentCloud = processor.SegmentPlane(latest_scan, 100, 0.2);
    renderPointCloud(viewer, segmentCloud.first,"obstaclesCloud",Color(1,0,0));
    renderPointCloud(viewer, segmentCloud.second, "extractedPlaneCloud", Color(0,1,0));

    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cloudClusters = processor.Clustering(segmentCloud.first, 1.0, 1.0, 30.0);
