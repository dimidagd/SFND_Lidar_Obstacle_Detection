...
std::vector<std::vector<float>> CreateRandomData(int n, int n_clusters, float sigma)
{
	...
	std::uniform_real_distribution<float> distribution(-xlim+4*sigma,xlim-4*sigma);
	
	std::vector<std::vector<float>> mu;

	for (int i=0; i < n_clusters ; i++)
	{
		std::vector<float> mu_tmp = {distribution(generator), distribution(generator)};
		mu.push_back(mu_tmp);
		std::normal_distribution<float> normal_distribution_x(mu_tmp[0], sigma);
		std::normal_distribution<float> normal_distribution_y(mu_tmp[1], sigma);
		for (int j=0; j<(int)(n/n_clusters) ; j++)
		{
			std::vector<float> sample = {normal_distribution_x(generator), normal_distribution_y(generator)};
...
int main ()
{
	...
	std::vector<std::vector<float>> points = CreateRandomData(1000, 50 ,0.01);
