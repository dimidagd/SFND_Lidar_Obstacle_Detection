...
pcl::PointCloud<pcl::PointXYZ>::Ptr CreateData(int numInliers, float percOutliers)
{
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>());
  	// Add inliers
  	float scatter = 0.6;
	
  	for(int i = -numInliers/2; i < numInliers/2; i++)
  	{
  		double rx = 2*(((double) rand() / (RAND_MAX))-0.5);
  		double ry = 2*(((double) rand() / (RAND_MAX))-0.5);
  		pcl::PointXYZ point;
  		point.x = 5*i/(numInliers/2.0)+scatter*rx;
  		point.y = 5*i/(numInliers/2.0)+scatter*ry;
  		point.z = 0;

  		cloud->points.push_back(point);
  	}
  	// Add outliers
  	int numOutliers = numInliers*percOutliers;
  	while(numOutliers--)
  	{
  		double rx = 2*(((double) rand() / (RAND_MAX))-0.5);
  		double ry = 2*(((double) rand() / (RAND_MAX))-0.5);
...
int main ()
{
	...
	int numInliers = 100;
	float percOutliers = 0.2;

	int numIters = 100;
	float distThreshold = 0.1;
	//pcl::PointCloud<pcl::PointXYZ>::Ptr cloud = CreateData(numInliers, percOutliers);
